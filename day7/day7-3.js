...
async function solveNumSplit() {
  ...
  for (let i = 1; i < pathsArray.length; i++) {
    for (let j = 0; j < pathsArray[i].length; j++) {
      // Check if there's a beam from directly above
      if (
        typeof pathsArray[i - 1][j] === "number" &&
        pathsArray[i - 1][j] > 0
      ) {
        const beamCount = pathsArray[i - 1][j];
        //the key I was missing as addign to a value the values above it is the there were splitters to the right or left
        if (pathsArray[i][j] === "^") {
          // Split to left and right on the next row
          if (i + 1 < pathsArray.length) {
            if (typeof pathsArray[i + 1][j - 1] === "number") {
              pathsArray[i + 1][j - 1] += beamCount;
            }
            if (typeof pathsArray[i + 1][j + 1] === "number") {
              pathsArray[i + 1][j + 1] += beamCount;
            }
